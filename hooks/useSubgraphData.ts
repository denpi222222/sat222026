/**
 * !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 * !! ВНИМАНИЕ! SUBGRAPH denis-3 НЕ РАБОТАЕТ! Возвращает "Not found"      !!
 * !! Этот хук возвращает null/пустые данные.                              !!
 * !! НУЖНО ПЕРЕПИСАТЬ НА ПРЯМЫЕ ЗАПРОСЫ К КОНТРАКТУ (как nft-inspector)  !!
 * !! Проверено: 2026-01-08. URL: denis-3/v0.0.1 = МЁРТВ                  !!
 * !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 */
'use client';
...
// Subgraph endpoint (Updated to correct version)
const SUBGRAPH_URL =
...
export const useSubgraphData = () => {
  ...
  const fetchContractStats = async () => {
    try {
      ...
      if (data.contractStats && data.contractStats.length > 0) {
        setContractStats(data.contractStats[0]);
      }
    } catch (err) {
  ...
  const fetchCRAAStats = async () => {
    try {
      ...
      if (data.craaStats && data.craaStats.length > 0) {
        setCraaStats(data.craaStats[0]);
      }
    } catch (err) {
      console.error('Failed to fetch CRAA stats:', err);
    }
  };
