...
import { getLocalNFTImage, hasLocalNFTImage, extractTokenId } from '@/lib/nftImages';
...
// FALLBACK: IPFS (if tokenId > 5000 or local not available)
export const getNFTImage = (nft: AlchemyNFT): string => {
  try {
    ...
    if (tokenId) {
      // ✅ PRIORITY 1: Local image (instant loading, no IPFS blocks)
      const localImage = getLocalNFTImage(tokenId);
      if (localImage !== '/favicon.ico') {
        return localImage; // /nft/0023.webp (instant!)
      }
    }
    
    // ❌ FALLBACK: IPFS (only if no local image)
    let imageUrl = '';
