...
export function NeonTitle({ title, subtitle }: NeonTitleProps) {
  ...
  return (
    <div ref={containerRef} className="relative z-10 text-center select-none">
      <h1
        className={
          'mx-auto uppercase tracking-[0.15em] ' +
      ...
      {subtitle ? (
        <div className="mt-4 md:mt-6 text-base md:text-xl lg:text-2xl crazycube-neon-subtitle">
          {subtitle}
        </div>
      ) : null}
      ...
      <style jsx>{`
        ...
        @keyframes crazycubeFlicker {
          0% { filter: brightness(0.6) saturate(0.9); letter-spacing: 0.13em; transform: translateY(0); }
          20% { filter: brightness(1.4) saturate(1.6); letter-spacing: 0.17em; transform: translateY(-1px); }
          40% { filter: brightness(0.8) saturate(1.0); letter-spacing: 0.15em; transform: translateY(0); }
          60% { filter: brightness(1.8) saturate(1.8); letter-spacing: 0.18em; transform: translateY(-1px); }
          80% { filter: brightness(0.7) saturate(1.0); letter-spacing: 0.14em; transform: translateY(0); }
          100% { filter: brightness(1.1) saturate(1.3); letter-spacing: 0.15em; transform: translateY(0); }
        }
      `}</style>
    </div>
  );
