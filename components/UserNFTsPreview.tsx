...
// Get NFT image - PRIORITY: Local from /public/nft/ (instant!)
const resolveImageSrc = (nft: any) => {
  try {
    // Extract tokenId from NFT object
    const tokenId = getTokenIdAsDecimal(nft);
    ...
    // ❌ Fallback (should not happen with 5000 local images)
    const url = nft.image || nft.metadata?.image;
