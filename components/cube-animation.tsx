...
export function CubeAnimation({ desktopScale = 1 }: CubeAnimationProps) {
  ...
  // Determine cube size - responsive sizing
  // Desktop: 170px (40% larger) | Mobile: 65px (fits 5 in a row nicely)
  let cubeSize = isMobile.isMobile ? 65 : 170;

  return (
    <motion.div
      key={languageKey} // Force re-render when language changes
      ref={containerRef}
      className={`relative w-full ${isMobile.isMobile ? 'h-[40vh]' : 'h-[400px]'} flex items-center justify-center overflow-hidden`}
    >
      ...
      <div className='relative flex flex-wrap items-center justify-center gap-2 md:gap-8 z-10'>
        ...
        {initialCubeIndices.length > 4 && (
          ...
          >
            <motion.div
              className={`absolute inset-0 rounded-full ${partyMode
      ...
      {isClient &&
        Array.from({ length: isMobile ? 15 : 30 }).map((_, i) => (
          <motion.div
            ...
            className={`absolute w-2 h-2 rounded-full ${i % 5 === 0
